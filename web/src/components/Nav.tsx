...
function useDestinations(): Destination[] {
  ...
  return [
    ...
    ...(printStore ? [{ dialog: "prints" as const, label: "Prints", icon: Layers }] : []),
...
export function HeaderActions() {
  ...
  return (
    <nav className="hidden items-center gap-2 lg:flex xl:gap-3" aria-label="Primary">
      {useDestinations().map(({ dialog, label, icon: Icon, primary, iconOnly }) => (
        <button
          key={dialog}
...
export function AppNav() {
  ...
  return (
    <nav className="app-nav" aria-label="Primary">
      {useDestinations().map(({ dialog, label, icon: Icon, primary }) => (
