...
export function CustomiseBar() {
  ...
  const panels = engine.plugins.filter((p) => p.enabled && p.manifest.surfaces.includes("panel"));
  const hiddenPanels = applyLayout(panels, section(engine.settings.layout, "plugins")).hidden;
  return (
    <div className="mx-auto max-w-[1500px] px-4 pt-5 sm:px-6">
      ...
      <HiddenTray
        label="HIDDEN PANELS"
        items={hiddenPanels.map((plugin) => ({ id: plugin.id, name: plugin.manifest.name }))}
        onShow={(id) => mutateLayout("plugins", (s) => toggleHidden(s, id))}
      />
    </div>
  );
}
